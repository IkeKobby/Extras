...
class Functions:
    ...
    def repeated_entries(x):
        ...
        L = len(np.unique(x)) # Get the length of different entries in x
        S = np.eye(N, L) # create a matrix of zeros and ones
        # To find `u` , I have to solve the matrix equation `S^-1 * x`. Since N > L, S has a left inverse. 
        S_inverse = np.linalg.inv(S.T @ S) @ S.T
        u = S_inverse @ x
        return S, u
...
def orth_vector(B):
        ...
        b = np.zeros(B.shape[1] + 1) # `b` zeros for orthogonal vectors
        b[-1] = 1 # Set the value for random vector to be non zero
        v = np.linalg.lstsq(B_stack, b, rcond=None) # Solve for `x` using A @ x = b 
        return v
